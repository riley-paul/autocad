...
class Plot:
    ...
    col_y: typing.Union[str, typing.List] = (
        "y"  # Column name of the y-axis values. Can be a list if multiple values
    )
    ...
    def __post_init__(self):
        ...
        self.ranges = (
            []
        )  # list of dataframes with columns of [text,colour,KP_beg,KP_end]
        self.range_height = self.text_height + self.over

        self.data = self.data.sort_values(self.col_x)
        pt_avg = Point(
            self.data[self.col_x].mean(), self.data[self.col_y].mean().mean()
        )
    ...
    def transform(self, x=None, y=None) -> float | Point:
        if x == None and y != None:
            return self.origin.y - self.y_min + y
        if x != None and y == None:
            return self.origin.x - self.x_min + x
        if x != None and y != None:
            return Point(x, y).copy(
                self.origin.x - self.x_min, self.origin.y - self.y_min
            )
    ...
    def draw_grid(self):
        ...
        while i <= self.x_ext + self.origin.x:
            ...
            if self.is_KP:
                txt_label = format_KP(value, False)
            else:
                txt_label = f"{value:.0f}"
            ...
            command += Text(
                txt_label, pt_txt, height=self.text_height, just="TC"
            ).ACAD()
        ...
        while i <= self.y_ext + self.origin.y:
            ...
            command += Text(
                txt,
                position=pt_min.copy(self.over, self.over),
                height=self.text_height,
                just="BL",
            ).ACAD()
            command += Text(
                txt,
                position=pt_max.copy(-self.over, self.over),
                height=self.text_height,
                just="BR",
            ).ACAD()
        ...
        if self.x_lab:
            pos = self.origin.copy(
                self.x_ext / 2,
                -(self.over * 3 + self.text_height + self.over)
                - self.range_height * len(self.ranges),
            )
            command += lay_label.ACAD()
            command += Text(
                self.x_lab, position=pos, height=self.text_height / 1.2, just="TC"
            ).ACAD()
        ...
        if self.y_lab:
            ...
            command += Text(
                self.y_lab,
                position=self.origin.copy(-self.over, self.y_ext / 2),
                rot=90,
                height=self.text_height / 1.2,
                just="BC",
            ).ACAD()
            command += Text(
                self.y_lab,
                position=self.origin.copy(self.x_ext + self.over, self.y_ext / 2),
                rot=90,
                height=self.text_height / 1.2,
                just="TC",
            ).ACAD()
        ...
        if self.p_lab:
            ...
            command += lay_label.ACAD()
            command += Text(
                self.p_lab, position=pos, height=self.text_height * 1.5, just="TC"
            ).ACAD()
    ...
    def draw_lines(self):
        if self.colours:
            assert len(self.datasets) == len(
                self.colours
            ), "Need a colour for every dataset"
        else:
            self.colours = [
                [randint(25, 255) for _ in range(3)] for _ in range(len(self.col_y))
            ]
        ...
        data = self.data[
            self.data[self.col_x].between(self.x_min + inset, self.x_max - inset)
        ]

        for y, colour in zip(self.col_y, self.colours):
            cleaned = data.dropna(subset=[self.col_x, y])
            points = cleaned.apply(
                lambda r: self.transform(r[self.col_x], r[y]), axis=1
            )
    ...
    def on_line(self, x, data_index=0):
        ...
        if intersections:
            return self.transform(intersections[0].x, intersections[0].y)
        else:
            return None
    ...
    def draw_range(self, ranges, top):
        ...
        command += Line(
            Point(self.transform(self.x_max), bot),
            Point(self.transform(self.x_min), bot),
        ).ACAD()
        ...
        for entry in ranges:
            ...
            if KP_beg >= self.x_max or KP_end <= self.x_min:
                continue
            ...
            command += Polygon(
                [Point(h1, bot), Point(h1, top), Point(h2, top), Point(h2, bot)]
            ).ACAD_solid()
